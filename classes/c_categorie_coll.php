...
class Categorie_coll {
    ...
    private function __construct2 ($selectArr, $orderArr) {
	    /*
	    	...
	    	1. Een array met selectiecriteria;
	    		...
	    		Als de array leeg is worden alle statusen geselecteerd.
    ...
    private function __construct3 ($selectArr, $orderArr, $limit) {
	    /*
	    	...
	    	2. Een array met de volgorde waarin de collection wordt aangeboden
	    		...
	    		attr 2: ASC of DESC
	    		De volgorde si op volgorde van de attributen. Dus sorteren op 1, daarbinnen op 2 enz.
	    	3. het aantal te lezen statusen
    ...
    private function buildQuery ($selectArr, $orderArr, $limit) {
	    ...
	    $this->query = 'SELECT categorie.* FROM categorie ';
// 		$this->query .= ' WHERE status.user_id = comm_user.id AND comm_status.deleted = FALSE AND comm_status.visible = TRUE AND comm_user.approved = TRUE ';
	    if (!empty($selectArr)) {
	    	$this->query .= ' AND ';
...
//				echo $categorie;
		...
		{
			  echo 'Connectie (categorie 1 in c_categorie_coll.php) met de database mislukt: ' . $e->getMessage();
			  return FALSE;
		}
