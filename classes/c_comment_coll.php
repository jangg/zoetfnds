...
/*
	static public function getNbrCommsNw($aanvraagid, $datum)
	{
		global $connection;
		try
		{
			openDB();
			$query = 'SELECT COUNT(*) FROM comment WHERE comment.delind = "n" AND comment.id_aanvraag = ' . $aanvraagid . ' AND comment.created > ' . $datum . ';';
			$stmt = $connection->prepare( $query );
			$number = $stmt->execute();
			return $stmt->fetchColumn();
		} catch (PDOException $e) 
		{
			  echo 'Connectie (comment 3 in c_comment_coll.php) met de database mislukt: ' . $e->getMessage();
			  return FALSE;
		}
	}
*/
